            ...
            if (!context.Articles.Any())
            {
                ...
                List<Article> articles = new List<Article>()
                {
                    ...
                    new Article()
                    {
                        ...
                        Title = "Stella",
                        Content =
                            "aStella is a Canadian wine grower and producer. It is located on the east bench of Osoyoos lake, British Columbia, only a few minutes from USA-Canada border. Southern Okanagan Valley is known for its many wineries because of its unique desert micro climate. In fact the Sonoran desert that starts in Mexico, ends some 20 kilometers after entering Canadian border.[1]"
                ...
                foreach (var article in articles)
                {
                    ...
                    {
                        article.CreatedOn =
                            DateTime.ParseExact("11-05-2005", "dd-MM-yyyy", CultureInfo.InvariantCulture);
                    }
                    else if (article.Title.Length % 4 == 0)
                    {
                        article.CreatedOn =
                            DateTime.ParseExact("30-01-1991", "dd-MM-yyyy", CultureInfo.InvariantCulture);
                    }
                    else if (article.Title.Length % 3 == 0)
                    {
                        article.CreatedOn =
                            DateTime.ParseExact("12-08-2010", "dd-MM-yyyy", CultureInfo.InvariantCulture);
                    }
                    else if (article.Title.Length % 2 == 0)
                    {
                        article.CreatedOn =
                            DateTime.ParseExact("23-10-2015", "dd-MM-yyyy", CultureInfo.InvariantCulture);
                    }
                    else
                    {
